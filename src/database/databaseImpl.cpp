...
bool DatabaseImpl::addUserToFriendList(
...
int DatabaseImpl::createReceiptInHistory(const bakcyl::core::Receipt& receipt) {
  ...
  for (const auto& element : receipt.items) {
    items.append(make_document(
        kvp("whoBuy", element.whoBuy), kvp("itemName", element.itemName),
        kvp("price", element.price), kvp("paid", false)));
  }
  ...
  if (receipt.mergedReceipts.size() > 0) {
    ...
    collection.insert_one(make_document(
        kvp("author", receipt.author), kvp("receiptName", receipt.receiptName),
        kvp("date", receipt.date), kvp("usersIncluded", usersIncluded),
        kvp("items", items), kvp("merged", false),
...
bool bakcyl::database::DatabaseImpl::changeIfMerged(
  ...
  collection.update_one(
      make_document(kvp("_id", document_id)),
      make_document(kvp("$set", make_document(kvp("merged", newState)))));
...
bakcyl::database::DatabaseImpl::getReceipts(const std::string& userID) {
  ...
  auto cursor = collection.find(make_document(
      kvp("$or",
          make_array(
              make_document(kvp("author", userID), kvp("merged", false)),
              make_document(kvp("usersIncluded",
                                make_document(kvp("$in", make_array(userID)))),
                            kvp("merged", false))))));
...
bool bakcyl::database::DatabaseImpl::paidForItem(const std::string& receiptID,
  ...
  auto cursor = collection.find_one(make_document(
      ...
      kvp("items",
          make_document(
              kvp("$elemMatch", make_document(kvp("whoBuy", whoBuy),
                                              kvp("itemName", itemName)))))));
  auto doc_view = cursor->view();
  if (!cursor) {
    return false;
  }
  ...
  auto array_value_items = doc_view["items"];
  for (auto& object : array_value_items.get_array().value) {
    if (object["whoBuy"].get_string().value.to_string() == whoBuy &&
        object["itemName"].get_string().value.to_string() == itemName &&
        object["paid"].get_bool().value == true) {
      return false;
    }
  }
      ...
      make_document(
          ...
          kvp("items",
              make_document(kvp("$elemMatch",
                                make_document(kvp("whoBuy", whoBuy),
                                              kvp("itemName", itemName)))))),
      make_document(kvp("$set", make_document(kvp("items.$.paid", true)))));
  return true;
}

std::vector<bakcyl::core::ItemToPay>
bakcyl::database::DatabaseImpl::getItemsToPay(const std::string& userID) {
...
std::string bakcyl::database::DatabaseImpl::getUserIDUsingUsername(
    const std::string& username) const {
  auto collection = database["users"];
